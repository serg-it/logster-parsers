...
class HAProxyPygster(PygsterParser):
    ...
    def get_state(self, duration):
        ...
        return [
            ...
            MetricObject("1xx.time", self.http_1x['time']/self.http_1x['requests'], metric_type='ms'),
            MetricObject("1xx.size", self.http_1x['bytes']/self.http_1x['requests'], metric_type='b'),
            MetricObject("2xx.count", self.http_2x['requests'], metric_type='c'),
            MetricObject("2xx.time", self.http_2x['time']/self.http_2x['requests'], metric_type='ms'),
            MetricObject("2xx.size", self.http_2x['bytes']/self.http_2x['requests'], metric_type='b'),
            MetricObject("3xx.count", self.http_3x['requests'], metric_type='c'),
            MetricObject("3xx.time", self.http_3x['time']/self.http_3x['requests'], metric_type='ms'),
            MetricObject("3xx.size", self.http_3x['bytes']/self.http_3x['requests'], metric_type='b'),
            MetricObject("4xx.count", self.http_4x['requests'], metric_type='c'),
            MetricObject("4xx.time", self.http_4x['time']/self.http_4x['requests'], metric_type='ms'),
            MetricObject("4xx.size", self.http_4x['bytes']/self.http_4x['requests'], metric_type='b'),
            MetricObject("5xx.count", self.http_5x['requests'], metric_type='c'),
            MetricObject("5xx.time", self.http_5x['time']/self.http_5x['requests'], metric_type='ms'),
            MetricObject("5xx.size", self.http_5x['bytes']/self.http_5x['requests'], metric_type='b'),
        ]
